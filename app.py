...
# Configuration
st.set_page_config(page_title="Flight Finder", layout="wide")
...
with st.sidebar:
    ...
    4. The app will find flight options and recommend the best one
    """)
...
if st.button("Search Flights", type="primary"):
    ...
    if not openai_api_key or not serper_api_key:
        ...
        with st.spinner("Searching for flights..."):
            # Get search results
            search_results = search_flights(origin, destination, travel_date, serper_api_key)
            
            if search_results:
                # Display the raw search data in a debug expander
                with st.expander("View raw search data"):
                    st.json(search_results)
                
                # Analyze with OpenAI
                with st.spinner("Analyzing flight options..."):
                    flight_analysis = analyze_flights(search_results, openai_api_key)
